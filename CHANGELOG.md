...
한글 뒤 괄호 짝이 어긋나는 문제 수정.

### 버그 수정

- **CJK 문맥 괄호 짝 깨짐**: `디지털 전환(DT)`처럼 한글 뒤에 라틴을 괄호로 묶으면 여는 괄호만 위로 올라가 짝이 어긋나던 문제 수정
- 원인: CJK `.case` 치환 룩업이 괄호를 한 글자씩 독립 판정했음. 여는 괄호는 앞의 한글을 보고 `parenleft.case`(중심 y 745)로 바뀌지만, 닫는 괄호는 앞이 라틴이고 뒤가 없어 원본(중심 y 651)으로 남아 94유닛 어긋남 — 40px 이상에서 눈에 보임
- 괄호류에 방향성을 부여해 해결: 여는 괄호는 **뒤에** CJK가 올 때만, 닫는 괄호는 **앞에** CJK가 있을 때만 치환
- `한글(가나)`처럼 양쪽이 CJK인 경우는 그대로 양쪽 치환, `전환(DT)`는 양쪽 모두 원본 유지 — 어느 쪽이든 짝이 맞음
- 소괄호·대괄호·중괄호 모두 적용 (21개 케이스 중 깨짐 10건 → 0건, 수직 중심 차이 94 → 0)
- `@`·`…` 등 방향성 없는 기호는 기존 대칭 규칙 유지, Inter 원본 라틴 동작(`ABC(DT)`, `T)`)도 무손상
- 1.0.0부터 있던 버그
